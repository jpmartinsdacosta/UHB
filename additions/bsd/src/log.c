...
#define BSD_RSYSLOG_CONFIG_CURRENT      "/root/uhb/base/config/current/rsyslog.conf"
#define BSD_RSYSLOG_REMOTE_CURRENT      "/root/uhb/base/config/current/50-default.conf"
...
void detect_rfc5424() {
    if(find_string_in_file(SEND_RFC5424,BSD_RSYSLOG_CONFIG_CURRENT)){
        rfc_5424_send = true;
    }else{
        rfc_5424_send = false;
    }
    if(find_string_in_file(WRITE_RFC5424,BSD_RSYSLOG_CONFIG_CURRENT)){
        rfc_5424_write = true;
    }else{
...
void apply_rfc5424() {
    ...
    if(opt == 0 && !find_string_in_file(SEND_RFC5424,BSD_RSYSLOG_CONFIG_CURRENT)){
        append_to_file(SEND_RFC5424,BSD_RSYSLOG_CONFIG_CURRENT);
        rfc_5424_send = true;
    }else if(opt == 1){
        find_and_replace(SEND_RFC5424,"",BSD_RSYSLOG_CONFIG_CURRENT);
        rfc_5424_send = false;
    }
    opt = get_yes_no_input("MSG 2/2: Use RFC5424 standard when storing logs in the system? (Y/N):");
    if(opt == 0 && !find_string_in_file(WRITE_RFC5424,BSD_RSYSLOG_CONFIG_CURRENT)){
        append_to_file(WRITE_RFC5424,BSD_RSYSLOG_CONFIG_CURRENT);
        rfc_5424_write = true;
    }else if(opt == 1){
        find_and_replace(WRITE_RFC5424,"",BSD_RSYSLOG_CONFIG_CURRENT);
        rfc_5424_write = false;
    }
}
...
void enable_udp_module(){
    ...
    find_and_replace("#module(load=\"imudp\")","module(load=\"imudp\")",BSD_RSYSLOG_CONFIG_CURRENT);
    find_and_replace("#input(type=\"imudp\" port=\"514\")",a,BSD_RSYSLOG_CONFIG_CURRENT);
}

void enable_tcp_module(){
    char b[MAX_LINE_LENGTH];
    snprintf(b,sizeof(b),"input(type=\"imtcp\" port=\"%s\")",tcp_port);
    find_and_replace("#module(load=\"imtcp\")","module(load=\"imtcp\")",BSD_RSYSLOG_CONFIG_CURRENT);
    find_and_replace("#input(type=\"imtcp\" port=\"514\")",b,BSD_RSYSLOG_CONFIG_CURRENT);
}
...
void disable_udp_module(){
    ...
    snprintf(a,sizeof(a),"input(type=\"imudp\" port=\"%s\")",udp_port);
    find_and_replace("module(load=\"imudp\")","#module(load=\"imudp\")",BSD_RSYSLOG_CONFIG_CURRENT);
    find_and_replace(a,"#input(type=\"imudp\" port=\"514\")",BSD_RSYSLOG_CONFIG_CURRENT);
}
...
void disable_tcp_module(){
    ...
    snprintf(b,sizeof(b),"input(type=\"imtcp\" port=\"%s\")",tcp_port);
    find_and_replace("module(load=\"imtcp\")","#module(load=\"imtcp\")",BSD_RSYSLOG_CONFIG_CURRENT);
    find_and_replace(b,"#input(type=\"imtcp\" port=\"514\")",BSD_RSYSLOG_CONFIG_CURRENT);
}
...
bool edit_udp_module(const char *port){
    ...
    find_and_replace("#module(load=\"imudp\")","module(load=\"imudp\")",BSD_RSYSLOG_CONFIG_CURRENT);
    return find_and_replace(current,replace,BSD_RSYSLOG_CONFIG_CURRENT);
}
...
void add_log_reception_rule(){
    ...
    append_to_file(line,BSD_RSYSLOG_CONFIG_CURRENT);
    append_to_file("*.* ?RemoteLogs",BSD_RSYSLOG_CONFIG_CURRENT);
    append_to_file("& ~",BSD_RSYSLOG_CONFIG_CURRENT);
}
